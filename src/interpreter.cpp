...
Value *Interpreter::evaluate_function(std::string identifier,
  ...
  if (identifier == "input") {
    if (parameters.size() != 0) {
      std::ostringstream msg;
      msg << "Too many parameters. Expected: 0, Actual: "
          << parameters.size();
  ...
  Value *Interpreter::evaluate(Node * node) {
    ...
    if (auto *vn = dynamic_cast<VariableNode *>(node)) {
      ...
      return child->apply_operator(
          un->token, nullptr); // Unary operation applies to child,
  ...
  void Interpreter::visit(Node * node) {
    ...
    if (auto *bn = dynamic_cast<BlockNode *>(node)) {
      ...
      if (vn->is_definition) {
        ...
        VariableNode *stored_variable = new VariableNode(variable->identifier, new TerminalNode(stored_value), false);
      ...
      if (is_assign(bnn->token.type)) {
        ...
        Value *stored_value = variable_value->apply_operator(
            assign_operator, evaluate(bnn->right));
        set_variable_value(variable->identifier.value, stored_value);
      } else {
        evaluate(bnn);
      }
